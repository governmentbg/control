...
<div class="ui segment">
            ...
            values.forEach(function (val) {
                cluster.addLayer(
                    L.marker(
                        ...
                        {
                            icon: L.divIcon({
                                className: 'map-marker ' + (!val.num ? 'map-device' : (val.real ? 'map-real' : ''))
                            })
                        }
                    )
                    .bindTooltip(
                        val.number + (val.num ? ' (' + val.num + ')' : ''),
                        {
                            permanent: false,
                            direction: 'top'
                        }
                    ...
                    .on('click', function () {
                        window.location = "<?= $this->e($url('devices')); ?>?udi=" + val.number
                    })
                );
            });
        }
    ...
    <style nonce="<?= $this->e($cspNonce) ?>">
        ...
        input[name="period"] { text-align:center !important; min-width:320px; }
        .submit.button { position:relative; top:2px; }
    </style>
</div>